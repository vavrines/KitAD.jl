...
GC.gc()
GC.@preserve res = sci_train(loss, p0, Adam(0.05); cb=cb, iters=100, ad=AutoZygote())
GC.@preserve res = sci_train(loss, res.u, AdamW(0.01); cb=cb, iters=100, ad=AutoZygote())
#
# ~ 0.30x
#
...
plot(ps.x[2:ps.nx], sigmoid.(res.u[2:ps.nx]))

using KitBase.JLD2
cd(@__DIR__)
u = res.u
@save "sod_multi.jld2" u
